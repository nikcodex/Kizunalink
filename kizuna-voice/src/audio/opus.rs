...
impl OpusEncoder {
    pub fn new() -> Result<Self> {
        let encoder = Encoder::new(SAMPLE_RATE, CHANNELS, Application::Audio).map_err(|e| {
            crate::error::Error::Transport(format!("Opus encoder init failed: {}", e))
        })?;
        ...
        match source {
            ...
            OpusSource::Pcm(pcm_data) => {
                if pcm_data.is_empty() {
                    ...
                    let len = self.encoder.encode(&silence, &mut output).map_err(|e| {
                        crate::error::Error::Transport(format!("Opus encode silence failed: {}", e))
                    })?;
                    output.truncate(len);
                    Ok(AudioFrame::Opus(output))
                } else {
                    let mut output = vec![0u8; MAX_PACKET_SIZE];
                    let len = self.encoder.encode(&pcm_data, &mut output).map_err(|e| {
                        crate::error::Error::Transport(format!("Opus encode failed: {}", e))
                    })?;
                    output.truncate(len);
                    Ok(AudioFrame::Opus(output))
                }
...
mod tests {
    ...
    fn test_encode_silence() {
        ...
        match frame {
            AudioFrame::Opus(data) => {
                ...
                assert!(
                    data.len() < 100,
                    "Silence frame should be small, got {} bytes",
                    data.len()
                );
            }
    ...
    fn test_passthrough_encoded() {
        ...
        let frame = encoder
            .encode(OpusSource::Encoded(fake_opus.clone()))
            .expect("passthrough");
        match frame {
