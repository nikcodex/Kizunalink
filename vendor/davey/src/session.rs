...
pub struct DaveSession {
    ...
    pending_encryptor_ratchets: HashMap<u16, HashRatchet>,
    pending_privacy_codes: HashMap<u16, String>,
}
...
impl DaveSession {
    ...
    pub fn new(
        ...
        Ok(DaveSession {
            ...
            pending_encryptor_ratchets: HashMap::new(),
            pending_privacy_codes: HashMap::new(),
        })
    }
    ...
    pub fn reinit(
        ...
        self.reinit_inner(protocol_version, user_id, channel_id, key_pair, false)
    }

    /// Re-initializes MLS while retaining active media ratchets until the voice
    /// gateway executes the associated transition.
    pub fn reinit_for_transition(
        &mut self,
        protocol_version: NonZeroU16,
        user_id: u64,
        channel_id: u64,
        ...
        self.reinit_inner(protocol_version, user_id, channel_id, key_pair, true)
    }
    ...
    fn reinit_inner(
        ...
        self.pending_encryptor_ratchets.clear();
        self.pending_privacy_codes.clear();
        if !preserve_media_ratchets {
    ...
    pub fn create_key_package(&mut self) -> Result<Vec<u8>, CreateKeyPackageError> {
        ...
        let key_package = KeyPackage::builder()
            ...
            .leaf_node_capabilities(
                dave_protocol_version_to_capabilities(self.protocol_version).unwrap(),
            )
    ...
    pub fn process_proposals(
                        ...
                        if let Proposal::Add(add_proposal) = proposal.proposal() {
                            let incoming_user_id = u64::from_be_bytes(
                                add_proposal
                                    ...
                                    .try_into()
                                    .map_err(
                                        ProcessProposalsError::CredentialContentConvertFailed,
                                    )?,
                            );
                            ...
                            {
                                return Err(ProcessProposalsError::UnexpectedUser(
                                    incoming_user_id,
                                ));
                            }
        ...
        let welcome = welcome.and_then(|message| match message.body() {
            MlsMessageBodyOut::Welcome(welcome) => Some(
                welcome
                    .tls_serialize_detached()
    ...
    pub fn process_welcome(&mut self, welcome: &[u8]) -> Result<(), ProcessWelcomeError> {
        self.process_welcome_inner(welcome, None)
    }

    /// Processes a welcome while staging the outbound ratchet until the voice
    /// gateway executes `transition_id`. Receive ratchets are updated immediately
    /// so in-flight frames from either epoch remain decryptable.
    pub fn process_welcome_for_transition(
        &mut self,
        welcome: &[u8],
        transition_id: u16,
    ) -> Result<(), ProcessWelcomeError> {
        self.process_welcome_inner(welcome, Some(transition_id))
    }
    ...
    fn process_welcome_inner(
        ...
        self.update_ratchets(transition_id)?;
    ...
    pub fn process_commit(&mut self, commit: &[u8]) -> Result<(), ProcessCommitError> {
        self.process_commit_inner(commit, None)
    }

    /// Processes a commit while staging the outbound ratchet until the voice
    /// gateway executes `transition_id`.
    pub fn process_commit_for_transition(
        &mut self,
        commit: &[u8],
        transition_id: u16,
    ) -> Result<(), ProcessCommitError> {
        self.process_commit_inner(commit, Some(transition_id))
    }
    ...
    fn update_ratchets(&mut self, transition_id: Option<u16>) -> Result<(), UpdateRatchetsError> {
        let group = self
            .group
            .as_ref()
        ...
        for member in group.members() {
            let Ok(user_id_bytes) =
                TryInto::<[u8; 8]>::try_into(member.credential.serialized_content())
            else {
        ...
        let encryptor_ratchet = self.get_key_ratchet(user_id)?;
        ...
        if let Some(transition_id) = transition_id.filter(|id| *id != 0) {
            self.pending_encryptor_ratchets
                .insert(transition_id, encryptor_ratchet);
            self.pending_privacy_codes
                .insert(transition_id, privacy_code);
        } else {
            self.pending_encryptor_ratchets.clear();
            self.pending_privacy_codes.clear();
            self.encryptor.set_key_ratchet(encryptor_ratchet);
    ...
    /// Activates the outbound ratchet staged for a gateway transition.
    pub fn execute_transition(&mut self, transition_id: u16) -> bool {
        let Some(ratchet) = self.pending_encryptor_ratchets.remove(&transition_id) else {
            return false;
        };

        self.encryptor.set_key_ratchet(ratchet);
        if let Some(privacy_code) = self.pending_privacy_codes.remove(&transition_id) {
            self.privacy_code = privacy_code;
        }
        true
    }

    /// Drops staged media state for a transition that will not execute.
    pub fn discard_transition(&mut self, transition_id: u16) {
        self.pending_encryptor_ratchets.remove(&transition_id);
        self.pending_privacy_codes.remove(&transition_id);
    }
    ...
    fn get_key_ratchet(&self, user_id: u64) -> Result<HashRatchet, UpdateRatchetsError> {
        ...
        trace!("Derived media key ratchet for user {user_id}");
    ...
    pub fn set_passthrough_mode(&mut self, passthrough_mode: bool, transition_expiry: Option<u32>) {
        for (_, decryptor) in self.decryptors.iter_mut() {
            decryptor.transition_to_passthrough_mode(
                passthrough_mode,
                transition_expiry.unwrap_or(10) as usize,
            );
        }
    }
}
...
mod transition_tests {
    ...
    const PACKET: &[u8] = &[0x78, 0x11, 0x22, 0x33, 0x44, 0x55];

    fn session_with_ratchet(secret: u8) -> DaveSession {
        let mut session = DaveSession::new(NonZeroU16::new(1).unwrap(), 1, 2, None).unwrap();
        session
            .encryptor
            .set_key_ratchet(HashRatchet::new(vec![secret; AES_GCM_128_KEY_BYTES]));
        session.is_ready = true;
        session
    }

    #[test]
    fn outbound_ratchet_changes_only_when_transition_executes() {
        let mut transitioning = session_with_ratchet(1);
        let mut old_epoch = session_with_ratchet(1);
        transitioning
            .pending_encryptor_ratchets
            .insert(42, HashRatchet::new(vec![2; AES_GCM_128_KEY_BYTES]));

        let before = transitioning.encrypt_opus(PACKET).unwrap();
        let expected_old = old_epoch.encrypt_opus(PACKET).unwrap();
        assert_eq!(before, expected_old);

        assert!(transitioning.execute_transition(42));
        let mut new_epoch = session_with_ratchet(2);
        let after = transitioning.encrypt_opus(PACKET).unwrap();
        let expected_new = new_epoch.encrypt_opus(PACKET).unwrap();
        assert_eq!(after, expected_new);
    }

    #[test]
    fn unknown_transition_does_not_consume_staged_ratchet() {
        let mut session = session_with_ratchet(1);
        session
            .pending_encryptor_ratchets
            .insert(42, HashRatchet::new(vec![2; AES_GCM_128_KEY_BYTES]));

        assert!(!session.execute_transition(7));
        assert!(session.pending_encryptor_ratchets.contains_key(&42));
    }
}
