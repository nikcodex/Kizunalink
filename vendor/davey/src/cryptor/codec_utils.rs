    ...
    while let Some((nal_unit_start_index, _start_code_size)) = nalu_index_pair.take() {
        ...
        if nal_type == NAL_TYPE_SLICE || nal_type == NAL_TYPE_IDR {
            ...
            processor.add_unencrypted_bytes(
                &frame[nal_unit_start_index..]
                    [..(NAL_UNIT_HEADER_SIZE + nal_unit_pps_bytes as usize)],
            );
...
pub fn process_frame_vp8(processor: &mut OutboundFrameProcessor, frame: &[u8]) -> bool {
    ...
    let Some(first_byte) = frame.first() else {
        warn!("VP8 frame is empty");
        return false;
    };

    let unencrypted_header_bytes: usize = if first_byte & 0x01 == 0 {
    ...
    if frame.len() < unencrypted_header_bytes {
        warn!("VP8 frame is too small to contain its payload header");
        return false;
    }
